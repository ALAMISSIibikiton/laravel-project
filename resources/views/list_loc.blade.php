    ...
    <div class=row>
      <div class="col w-100">
           <div class=" bg-primary d-flex align-items-center">
                <h3 class="card-title flex-grow-1"><i class="fas fa-users fa-1x"></i>Liste des locations</h3>

                <div class=" d-flex align-items-center">
                  <a class="btn btn-link text-white mr-4 d-block" href="{{ route('create_loc')}}"> <i class="fas fa-user-plus">
                  </i> Nouveau bien</a>
     ...
     <div>
        ...
        <div class="d-flex justify-content-end ">
             <div class="form-group pr-3 mr-4 p-4">
                <a href="{{route('index_loc2')}}">
                <button type="button" class="btn btn-info">liste suivante</button></a>
             </div>
  ...
  <table class="table table-striped mh-100">
    ...
    <thead>
        <tr>
          ...
          <th style="font-weight:bold">Nombre de jardin</th>
          <th style="font-weight:bold">Nombre de parking</th>
          <th style="font-weight:bold">Nombre d'appartement</th>
    ...
    <tbody>
        ...
        <tr>
            <td >{{$location->nom_pro}}</td>
            <td>{{$location->tel_pro}}</td>
            <td>{{$location->ville_lo}}</td>
            <td>{{$location->quartier_lo}}</td>
            <td>{{$location->nbr_jardin}}</td>
            <td>{{$location->nbr_parking}}</td>
            <td>{{$location->nbr_appart}}</td>
            <td style="margin-left:800px">
               <form action="{{route('destroy_loc', $location->id)}}" method="post">
                  @csrf
                  @method('DELETE')
                  <a href="{{ route('edit_loc', $location->id)}}" class="btn btn-warning"><i class="fas fa-edit"></i></a>
